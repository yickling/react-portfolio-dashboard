import { useReducer, useMemoCompare, useEffect } from "react";
...
export function useFirestoreQuery(query) {
  ...
  const queryCached = useMemoCompare(query, (prevQuery) => {
    return prevQuery && query && query.id === prevQuery.id;
  });
...
export function getCollectionData(collection) {
  return null// collection.docs.map(getDocData);
}